...
import com.heap.IndexMinHeap;

// Dijkstra解决单源最短路径问题，前提是不能有负权边

public class Dijkstra <Weight extends Number & Comparable> {
	private WeightedGraph G;  //图的引用
	private int s;            //起始点
	private Number[] distTo;  //distTo[i]存储从起始点s到i的最短路径
	private boolean[] marked; //标记数组，在算法运行过程中标记节点i是否被访问
	private Edge<Weight>[] from; //from[i] 记录最短路径中，到达i点的边是哪一条边
	
	//构造函数，使用 dijkstra算法求最短路径
	public Dijkstra(WeightedGraph graph,int s){
		...
		while(!ipq.isEmpty()){
			...
			for(Object item: G.adj(v)){
				...
				//如果从s点到w点的最短路径还没有找到
				if( !marked[w]){
					//如果w点以前没有访问过或者访问过，但通过当前的v点到w点距离更短，则进行更新
